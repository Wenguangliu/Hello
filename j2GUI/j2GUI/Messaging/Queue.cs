...
{
    /// <summary>
    /// This queue caters for getting the queue and etting the input receiver queue name. Whenever Gui process sends
    /// a message to the message queue, it is sent to the queue whose name is set in MessageReceiverName
    /// </summary>
    public class Queue
    {
        public static string MessageReceiverName = new string(StartupMessages.STARTUP_QUEUE_NAME);//queue name used by Gui for sending Gui messages only. This is set whenever Gui receives MSG_GUI_SET_INPUT_RECEIVER message. Not used for logging error
