...
namespace System.MessagingCE {
	public class P2PMessageQueue {
		...
		private const int DefaultMaxMsgLength = 4096 * 2; //you can change this default or override in the ctor
		...
		public ReadWriteResult Receive(Message msg, int timeoutMillis){
			...
			}else{//Receive failed, get extended info and map it to our returned enum
				...
				if (res == Win32Api.WAIT_OBJECT_0){
					...
					if (DataOnQueueChanged != null){
						if (this.CanRead //catches the case where a writer gets created/closed
								&& this.MessagesInQueueNow > 0){ //sideeffect: the event is not raised for Alert messages
