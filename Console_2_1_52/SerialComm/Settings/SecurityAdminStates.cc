...
Enter()
{
	...
	strcpy( data->buttonText[0], strings[ MSTR_ADD_ADMINISTRATOR ] );
...
Enter()
{
	...
	memset( data, 0, sizeof( GuiMiddleButtonsData_t ) );
	strcpy( data->buttonText[0], strings[ MSTR_ADMINISTRATOR_ID ] );
...
Enter()
{
	...
	sprintf( buff, "%s %s", strings[ TSTR_ADMINISTRATOR ], expiry->id );
	SetTopLabel( buff );
...
//	strcpy( data->buttonText[0], strings[ MSTR_CHANGE_EXPIRATION ] );
	strcpy( data->buttonText[0], strings[ MSTR_CHANGE_PASSWORD ] );
	strcpy( data->buttonText[1], strings[ MSTR_DELETE_ADMINISTRATOR ] );
...
Enter()
{
	...
	sprintf( data->message, "%s %s", expiry->id, strings[ ESTR_REMOVE_FROM_ADMIN_LIST ] );
