...
//		{ "log",				&hardwareData_m->tests.log						},
	...
	if ( inAssemblies_m )
	{
		...
		{
			if ( stricmp( paramName_m, p->paramName ) == 0 )
			{
				if ( attributes_m[0] != 0 && stricmp( attributes_m[0], "date" ) == 0 && attributes_m[1] != 0 )
				{
...
StartTag( void *data, const char *el, const char **attr )
{
	...
	if ( inData_m )
	{
		if ( stricmp( el, "Assemblies" ) == 0 )
		{
			inAssemblies_m = true;
		}

		else if ( stricmp( el, "Tests" ) == 0 )
		{
			inTests_m = true;
		}
...
EndTag( void *data, const char *el )
{
	...
	if ( inData_m )
	{
		...
		else if ( stricmp( el, "Assemblies" ) == 0 )
		{
