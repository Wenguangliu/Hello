...
Enter()
{
	...
	sprintf( data->buttonText[0], "%s: %s", strings[ MSTR_LAST_MODIFIED ], analytes[ selectedAnalyte ] );
	strcpy( data->buttonText[1], strings[ MSTR_ALL ] );
...
Enter()
{
	...
	msg.msgId = MSG_GUI_MIDDLE_BUTTONS_INFO;
	memset( data, 0, sizeof( GuiMiddleButtonsInfoData_t ) );
	strcpy( data->buttonText[0], strings[ MSTR_ANALYTE ] );
	strcpy( data->buttonText[1], strings[ MSTR_SPECIES ] );
	data->buttonHighlighted[0] = true;
	data->buttonHighlighted[1] = false;
...
Enter()
{
	...
	msg.msgId = MSG_GUI_MIDDLE_LIST;
		...
		{
			strcpy( data->items[ i ][0], analytes[ analyteList->list[ topItemListAnalytes + i ] ] );
		}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	{
		strcpy( data->items[0][0], strings[ MSTR_ADD_SPECIES ] );
		strcpy( data->items[1][0], strings[ MSTR_REMOVE_SPECIES ] );
		strcpy( data->items[2][0], strings[ MSTR_FACTORY_DEFAULT_SPECIES ] );
		i = 3;
	}
	else if ( topItemListSpecies == 1 )
	{
		strcpy( data->items[0][0], strings[ MSTR_REMOVE_SPECIES ] );
		strcpy( data->items[1][0], strings[ MSTR_FACTORY_DEFAULT_SPECIES ] );
		i = 2;
	}
	else if ( topItemListSpecies == 2 )
	{
		strcpy( data->items[0][0], strings[ MSTR_FACTORY_DEFAULT_SPECIES ] );
		i = 1;
	}
			
	for ( ; i < 5 && i < activeRanges->len - topItemListSpecies + 3; i++ )
	{
		strcpy( data->items[ i ][0], species[ activeRanges->list[ topItemListSpecies + i - 3 ] ] );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	{
		strcpy( data->buttonText[ i ], species[ nextSpecies ] );
		i++;
	}
	if ( nextControl != STYPE_NONE )
	{
		strcpy( data->buttonText[ i ], species[ nextControl ] );
		i++;
	}

	if ( factoryData->abaxisProductCode == VETSCAN_PRODUCT_CODE )
	{
		strcpy( data->buttonText[ i ], strings[ MSTR_SELECT ] );
	}
...
Enter()
{
	...
	{
		strcpy( data->items[ i ][0], species[ groupList->list[ i + topItemPickName ] ] );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
ProcessMessage( int msgId, char * data )
{
	...
	if ( doUpdate )
	{
		...
		for ( i = 0; i < 5 && i < groupList->len - topItemPickName; i++ )
		{
			strcpy( d->items[ i ][0], species[ groupList->list[ i + topItemPickName ] ] );
		}
		d->isSelectable = 1;
		d->hasUpDown = 1;
...
Enter()
{
	...
	if ( (factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE) &&
		 ((selectedSampleType < STYPE_CONTROL) || (selectedSampleType > STYPE_LAST_CONTROL)) )
	{
		sprintf( buff, "%s %s %s", strings.Gender( gender ), species[ selectedSampleType ], strings[ TSTR_RANGES ] );
	}
	else
	{
		sprintf( buff, "%s %s", species[ selectedSampleType ], strings[ TSTR_RANGES ] );
	}
	...
	for ( i = 0; i < 5 && i < analyteList->len - topItemListAnalyteRanges; i++ )
	{
		...
		strcpy( data->items[ i ][0], analytes[ analyte ] );
		FormatRefRange( data->items[ i ][1],
						refRanges->ranges[ selectedSampleType ][ analyte ][ gender-1 ],
						units[ analyte ] );
		strcpy( data->items[ i ][2], units.String( analyte ) );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	if ( factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE )
	{
		sprintf( buff, "%s %s %s", strings.Gender( gender ), analytes[ selectedAnalyte ], strings[ TSTR_RANGES ] );
	}
	else
	{
		sprintf( buff, "%s %s", analytes[ selectedAnalyte ], strings[ TSTR_RANGES ] );
	}
	SetTopLabel( buff );
	...
	for ( i = 0; i < 5 && i < activeRanges->len - topItemListSpeciesRanges; i++ )
	{
		...
		strcpy( data->items[ i ][0], species[ sampleType ] );
		FormatRefRange( data->items[ i ][1],
						refRanges->ranges[ sampleType ][ selectedAnalyte ][ gender-1 ],
						units[ selectedAnalyte ] );
		strcpy( data->items[ i ][2], units.String( selectedAnalyte ) );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	sprintf( buff, "%s %s [%s]", analytes[ selectedAnalyte ],
								 species[ selectedSampleType ],
								 units.String( selectedAnalyte ) );
	SetTopLabel( buff );
	
	msg.msgId = MSG_GUI_MIDDLE_ENTER_RANGE;
	strcpy( data->lowerLabel, strings[ MSTR_LOWER ] );
	strcpy( data->upperLabel, strings[ MSTR_UPPER ] );
	...
	strcpy( data->buttonText[0], strings[ MSTR_CLEAR ] );
	strcpy( data->buttonText[1], strings[ MSTR_DEFAULT ] );
...
Enter()
{
	...
	{
		strcpy( data->items[ i ][0], species[ list[ topItemSelectSpeciesLabel + i ] ] );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	{
		strcpy( data->items[ i ][0], species[ activeRanges->list[ topItemRemoveSpecies + i ] ] );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	sprintf( buff, "%s %s", species[ selectedSampleType ], strings[ ESTR_REMOVE_FROM_TYPE_LIST ] );
...
Enter()
{
	...
	strcpy( guiMsg.data.middleMessage.message, strings[ MSTR_PRESS_YES_REF_RANGES ] );
