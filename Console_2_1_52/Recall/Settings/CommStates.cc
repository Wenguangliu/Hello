...
LoadFuseSettings()
{
	...
	settingsData->astmOrderMethod = ASTM_ORDERS_WORK_LIST;
}
...
Enter()
{
	...
	if ( (factoryData->abaxisProductCode == VETSCAN_PRODUCT_CODE) &&
		 MountThumbDrive() && (stat( FUSE_SETTINGS_FILE_NAME, &statBuf ) == 0) )
	{	
		SetTopLabel( strings[ TSTR_SET_PROTOCOL ] );

		msg.msgId = MSG_GUI_MIDDLE_BUTTONS;
		memset( data, 0, sizeof( GuiMiddleButtonsData_t ) );
		strcpy( data->buttonText[0], strings[ MSTR_LOAD_FUSE_SETTINGS ] );
		strcpy( data->buttonText[1], strings[ MSTR_MANUAL_SETTINGS ] );
