...
Enter()
{
	...
	strcpy( data->buttonText[0], strings[ MSTR_CONFIGURE ] );
	strcpy( data->buttonText[1], strings[ MSTR_TEST ] );
...
Enter()
{
	...
	memset( data, 0, sizeof( GuiMiddleButtonsInfoData_t ) );
	strcpy( data->buttonText[0], strings[ MSTR_SET_DEFAULT ] );
	strcpy( data->buttonText[1], strings[ MSTR_SELECT_REPORTS ] );
...
Enter()
{
	...
	strcpy( data->buttonText[0], strings[ MSTR_ANALYZER_PRINTER ] );
	strcpy( data->buttonText[1], strings[ MSTR_EXTERNAL_PRINTER ] );
		...
		case MSG_GUI_OUT_BUTTON:
			switch ( idata )
			{
				case 0:		// Internal Printer
					errNum = ResultsPrint::TestInternalPrinter();
					break;
				case 1:		// External Printer
					errNum = ResultsPrint::TestExternalPrinter();
					break;
			}
			if ( errNum != SERR_NONE )
			{
...
Enter()
{
	...
	sprintf( data->buttonText[0], "%s: %d", strings[ MSTR_RESULTS ], settingsData->printNumResults );
	sprintf( data->buttonText[1], "%s: %d", strings[ MSTR_IQC ], settingsData->printNumIqc );
	sprintf( data->buttonText[2], "%s: %d", strings[ MSTR_ERRORS ], settingsData->printNumError );
	if ( settingsData->errorPrintAuto )
	{
		strcat( data->buttonText[2], " " );
		strcat( data->buttonText[2], strings[ MSTR_AUTO ] );
	}		
...
Enter()
{
	...
	strcpy( data->button0Text, strings[ MSTR_CLEAR ] );
...
Enter()
{
	...
	strcpy( data->button0Text, strings[ MSTR_CLEAR ] );
	strcpy( data->button1Text, strings[ MSTR_DEFAULT ] );
	strcpy( data->checkButtonText[0], strings[ MSTR_AUTO ] );
	strcpy( data->checkButtonText[1], strings[ MSTR_ALWAYS ] );
