...
Enter()
{
	...
	strcpy( data->buttonText[0], strings[ MSTR_SCREEN_CLICKS ] );
	strcpy( data->buttonText[1], strings[ MSTR_ALERTS ] );
	strcpy( data->buttonText[2], strings[ MSTR_STATUS ] );
...
Enter()
{
	...
	sprintf( data->currentValue, "%d%%", systemSetting );
	sprintf( data->newValue, "%d%%", systemSetting );
	strcpy( data->button0Text, strings[ MSTR_OFF ] );
...
ProcessMessage( int msgId, char * data )
{
	...
	if ( doUpdate )
	{
		...
		sprintf( msg.data.middleEnterValue.newValue, "%d%%", settingsData->touchClickVolume );
		SendGuiMessage( &msg, sizeof( GuiMiddleEnterValueData_t ) );

		// Do a "reference" click.
		msg.msgId = MSG_GUI_SOUND_SET_CLICK_VOLUME;
		msg.data.soundData.volume = settings->touchClickVolume;
		SendGuiMessage( &msg, sizeof( GuiSoundData_t ) );
		SendGuiMessage( MSG_GUI_SOUND_CLICK );
...
ProcessMessage( int msgId, char * data )
{
	...
	{
		msg.msgId = MSG_GUI_UPDATE_ENTER_VALUE_NEW;
		sprintf( msg.data.middleEnterValue.newValue, "%d%%", settingsData->alertVolume );
		SendGuiMessage( &msg, sizeof( GuiMiddleEnterValueData_t ) );

		msg.msgId = MSG_GUI_SOUND_SET_ALERT_VOLUME;
		msg.data.soundData.volume = settings->alertVolume;
		SendGuiMessage( &msg, sizeof( GuiSoundData_t ) );
...
ProcessMessage( int msgId, char * data )
{
	...
	{
		msg.msgId = MSG_GUI_UPDATE_ENTER_VALUE_NEW;
		sprintf( msg.data.middleEnterValue.newValue, "%d%%", settingsData->statusVolume );
		SendGuiMessage( &msg, sizeof( GuiMiddleEnterValueData_t ) );

		msg.msgId = MSG_GUI_SOUND_SET_STATUS_VOLUME;
		msg.data.soundData.volume = settings->statusVolume;
		SendGuiMessage( &msg, sizeof( GuiSoundData_t ) );
