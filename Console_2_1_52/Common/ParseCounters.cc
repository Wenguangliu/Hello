...
SetParam()
{
	if ( stricmp( paramName_m, "rotorRunCount" ) == 0 )
	{
		counterData_m->rotorRunCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "rotorAbortCount" ) == 0 )
	{
	...
	else if ( stricmp( paramName_m, "printCount" ) == 0 )
	{
		counterData_m->printCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "flashCount" ) == 0 )
	{
		counterData_m->flashCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "missedFlashCount" ) == 0 )
	{
		counterData_m->missedFlashCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "camCycleCount" ) == 0 )
	{
		counterData_m->camCycleCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "drawerCycleCount" ) == 0 )
	{
		counterData_m->drawerCycleCount = (int)atoi( paramValue_m );
	}
	
	else if ( stricmp( paramName_m, "spindleRevolutionCount" ) == 0 )
	{
...
ParseCounters( CounterData_t * counterData )
{
	 ...
	 */
	fp = fopen( COUNTERS_CONFIG_FILE_NAME, "r" );
	if ( fp == NULL )
	{
...
WriteCounters( CounterData_t * counterData )
{
	...
	// Open the file.
	fp = fopen( COUNTERS_CONFIG_FILE_NAME, "w" );
	if ( fp == 0 )
	{
		return CFG_ERR_FILE_OPEN;
	}
