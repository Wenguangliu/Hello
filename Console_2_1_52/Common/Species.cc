...
const char *
Species::
English( SampleType_t sampleType )
{
	static char	buff[ 100 ];
	...
	{
		sprintf( buff, "%s %d", strings.English( SSTR_CONTROL ), sampleType - STYPE_FIRST_CONTROL + 1 );
		return buff;
	}
	else if ( sampleType >= STYPE_FIRST_SPECIES && sampleType <= STYPE_LAST_SPECIES )
	{
		sprintf( buff, "%s %d", strings.English( SSTR_SPECIES ), sampleType - STYPE_FIRST_SPECIES + 1 );
		return buff;
	}
	else if ( sampleType >= STYPE_FIRST_NAMED_SPECIES && sampleType <= STYPE_LAST_NAMED_SPECIES )
	{
