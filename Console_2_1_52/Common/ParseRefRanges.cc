...
SetParam()
{
	...
	{
		if ( stricmp( paramName_m, "lastModifiedAnalyte" ) == 0 )
		{
			refRanges_m->lastModifiedAnalyte = GetAnalyteFromTag( paramValue_m );
		}
		else if ( stricmp( paramName_m, "lastModifiedSampleType" ) == 0 )
		{
...
GLD - we could just set a variable with the analyte ID here?
	...
	if ( inData_m )
	{
		...
		{
			if ( stricmp( attr[0], "gender" ) == 0 )
			{
				if ( stricmp( attr[1], "Male" ) == 0 )
				{
					gender_m = GENDER_MALE;
				}
				else if ( stricmp( attr[1], "Female" ) == 0 )
				{
					gender_m = GENDER_FEMALE;
				}
			}
			else if ( attr[2] != 0 && stricmp( attr[2], "gender" ) == 0 && attr[3] != 0 )
			{
				if ( stricmp( attr[3], "Male" ) == 0 )
				{
					gender_m = GENDER_MALE;
				}
				else if ( stricmp( attr[3], "Female" ) == 0 )
				{
					gender_m = GENDER_FEMALE;
				}
			}
		...
		else
		{
			...
			// Species
			else if ( stricmp( el, "Species" ) == 0 )
			{
				if ( attr[0] != 0 && attr[1] != 0 )
				{
					if ( stricmp( attr[0], "index" ) == 0 )
					{
						inSampleType_m = true;
						sampleType_m = (SampleType_t)(STYPE_OTHER + atoi( attr[1] ));
						refRanges_m->isDefined[ sampleType_m ] = true;
					}
					else if ( stricmp( attr[0], "name" ) == 0 )
					{
...
GetSampleTypeFromTag( const char * tag )
{
	...
	else
	{
		for ( i = 0; sampleMatches[ i ].sampleType != STYPE_NONE; i++ )
		{
			if ( stricmp( sampleMatches[ i ].tag, tag ) == 0 )
			{
...
GetAnalyteFromTag( const char * tag )
{
	...
	for ( i = 0; analyteMatches[ i ].analyte != AID_NONE; i++ )
	{
		if ( stricmp( analyteMatches[ i ].tag, tag ) == 0 )
		{
