...
GetAdmin( char * id )
{
	...
	{
		if ( stricmp( expiryData->admins[ i ].id, id ) == 0 )
		{
...
GetOp( char * id )
{
	...
	{
		if ( stricmp( expiryData->ops[ i ].id, id ) == 0 )
		{
...
DeleteOp( char * id )
{
	...
	for ( i = 0; i < expiryData->numOps; i++ )
	{
		if ( stricmp( expiryData->ops[ i ].id, id ) == 0 )
		{
			...
			{
				memcpy( &expiryData->ops[ i ], &expiryData->ops[ i+1 ], sizeof( Expiry_t ) );
			}
			expiryData->numOps--;
			break;
		}
...
CfgError_t
Expirys::
SetControlExpiration( time_t date )
{
