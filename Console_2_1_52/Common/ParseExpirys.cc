...
SetParam()
{
	if ( inParam_m )
	{
		if ( (stricmp( paramName_m, "control" ) == 0) && (expiry_m == 0) )
			{
				expiryData_m->controlExpiry = atoi( paramValue_m );
		}
		else if ( expiry_m != 0 )
		{
			if ( stricmp( paramName_m, "id" ) == 0 )
			{
				strcpy( expiry_m->id, paramValue_m );
			}
			else if ( stricmp( paramName_m, "password" ) == 0 )
			{
				strcpy( expiry_m->password, paramValue_m );
			}
			else if ( stricmp( paramName_m, "expiry" ) == 0 )
			{
				expiry_m->expiration = atoi( paramValue_m );
			}
		}
	}
}
...
ParseExpirys( ExpiryData_t * expiryData )
{
	 ...
	 * Open the XML file.
	 */
	fp = fopen( EXPIRYS_CONFIG_FILE_NAME, "r" );
	if ( fp == NULL )
	{
...
WriteExpirys( ExpiryData_t * expiryData )
{
	...
	// Open the file.
	fp = fopen( EXPIRYS_CONFIG_FILE_NAME, "w" );
	if ( fp == 0 )
	{
		return CFG_ERR_FILE_OPEN;
	}
	...
	sprintf( dataBuff, "%d", expiryData->controlExpiry );
	fprintf( fp, "    <control>%s</control>\n", dataBuff );
	crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );
	...
	for ( i = 0; i < expiryData->numAdmins; i++ )
	{
		...
		fprintf( fp, " <password>%s</password> ", dataBuff );
		crc = Crc32Update( crc, (unsigned char *)expiryData->admins[ i ].password, strlen( expiryData->admins[ i ].password ) );

		sprintf( dataBuff, "%d", expiryData->admins[ i ].expiration );
		fprintf( fp, " <expiry>%s</expiry>\n", dataBuff );
		crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );
	...
	for ( i = 0; i < expiryData->numOps; i++ )
	{
		...
		sprintf( dataBuff, "%d", expiryData->ops[ i ].expiration );
		fprintf( fp, " <expiry>%s</expiry>\n", dataBuff );
		crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );
