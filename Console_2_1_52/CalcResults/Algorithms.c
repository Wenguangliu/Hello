...
int main( int argc, char **argv )
{
	...
	inputQueue = mq_open( CALC_RESULTS_QUEUE_NAME, O_RDONLY );
	...
	if ( mq_getattr( inputQueue, &mqstat ) == -1 )
	{
	...
	analysisQueue = mq_open( ANALYSIS_QUEUE_NAME, O_WRONLY );
	...
	while ( 1 )
	{
		receivedSize = mq_receive( inputQueue, (char *)&receivedMsg, mqstat.mq_msgsize, NULL );
			...
			if ( receivedSize > 0 )
			{
				switch (receivedMsg.msgId )
				{
					...
					case MSG_CALC_RESULTS_DAC_TRIMS_COMPLETE:
						...
						mq_send( analysisQueue, (const char *)&analysisMsg, sizeof( int ), MQ_PRIO_MAX-1 );
						break;
					...
					case MSG_CALC_RESULTS_ANALYSIS_DONE:
						...
						mq_send( analysisQueue, (const char *)&analysisMsg, sizeof( AnalysisMsg_t ), MQ_PRIO_MAX-1 );
				...
				if ( errorNumber != 0 ) 
				{
					...
					analysisMsg.idata = errorNumber;
					mq_send( analysisQueue, (const char *)&analysisMsg, 2 * sizeof( int ), MQ_PRIO_MAX-1 );
