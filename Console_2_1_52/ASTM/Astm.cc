...
#define RECONNECT_INTERVAL			15							// TCP/IP socket reconnect interval in seconds
...
SocketConnect()
{
	...
	if ( difftime( time( 0 ), lastConnectAttempt ) > RECONNECT_INTERVAL )
	{
		...
		else
		{
			...
			if ( bind( fd_m, (struct sockaddr *)&sin, sizeof( sin ) ) == -1 )
			{
			...
			{			
				...
				{
					t.tv_sec = ASTM_RX_TCPIP_TIMEOUT_SEC;
					t.tv_usec = ASTM_RX_TCPIP_TIMEOUT_US;
					setsockopt( fd_m, SOL_SOCKET, SO_RCVTIMEO, &t, sizeof( struct timeval ) );
...
ReadPort( void * buffer )
{
	...
	if ( settingsData->comm.protocol == COMM_TCPIP )
	{
		...
		if ( ((readCount < 0) && (errno == ECONNRESET)) ||	// Connection reset
			 (readCount == 0) )								// Connection lost
...
WritePort( char * ch, int count )
{
	...
	if ( sent < count )
	{
		if ( writeOkLastTime )
		{
			LogError( "write() failed, errno = %d", errno );
...
unsigned char ProcessTxEscMethod(char escMethod, unsigned char checksum)
{
	...
	// E ASTM 1394-97: 15. Message Manufacturer Type Record(Requested Abaxis AS: Analyzer Status or PO: Pending Orders)   (variable fields)       
		...
		case ASTM_TX_M_RECORD_IDENTIFIER:					// 15.1.3        
			methodTextPtr = abaxisSendParam3_m;							// Abaxis record identifier = "AS" or "PO"        				
			break;
	...
	// E ASTM 1394-97: Order Data Invalid P Record / O Record / C Record methods       
		...
		case ASTM_TX_INVALID_O_REPORT_TYPE:					// 9.4.26  (always X for invalid order)
			methodText[0] = 'X';							        				
			break;
...
void	ProcessAbaxisAstmMsgs()
{
	...
	switch ( abaxisAstmRxMsgToProcess_m )
	{
		...
		case ASTM_RX_MFG_MSG:
			// set up for abaxis astm manufacturing status request message processing        
...
AstmState_t ProcessAstmState(AstmState_t astmState, unsigned short *errorNumber)
{
	...
	//	--- ASTM 1381-02 ABAXIS INSTRUMENT AS SENDING DEVICE STATES ---
		...
		case ASTM_TX_WAITING_ACK_STATE:
			...
			{
				astmTxRetryCount_m++;												// increment retry counter       
