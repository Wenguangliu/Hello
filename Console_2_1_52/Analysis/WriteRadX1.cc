								...
								RunInfo_t *			runInfo )
{
	...
	// barcodeTransitions
	bcFp = fopen( BC_TRANSITIONS_FILE, "w+" );
	if ( bcFp == 0 )
	{
	...
	// sampleType
	WriteString( species.English( (SampleType_t)runInfo->sampleType ), 15, fp );
	...
	CrcWrite( &runInfo->dateOfBirth.month, sizeof( char ), 1, fp );			// 0 - 11
	CrcWrite( &runInfo->dateOfBirth.day, sizeof( char ), 1, fp );			// 1 - 31
	
	// lastVaccination
	CrcWrite( &runInfo->lastVaccination.year, sizeof( short ), 1, fp );		// 0 - 9999
	CrcWrite( &runInfo->lastVaccination.month, sizeof( char ), 1, fp );		// 0 - 11
