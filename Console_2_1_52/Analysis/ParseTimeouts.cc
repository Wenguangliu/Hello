...
SetParam()
{
	...
	if ( stricmp( paramName_m, "opticalDacs" ) == 0 )
	{
		timeoutData_m->opticalDacs = value;
	}
	
	else if ( stricmp( paramName_m, "beadCheck1" ) == 0 )
	{
		timeoutData_m->beadCheck1 = value;
	}
	
	else if ( stricmp( paramName_m, "beadCheck2" ) == 0 )
	{
		timeoutData_m->beadCheck2 = atoi( paramValue_m );
	}

	else if ( stricmp( paramName_m, "distributionCheck" ) == 0 )
	{
		timeoutData_m->distributionCheck = value;
	}

	else if ( stricmp( paramName_m, "mixDone" ) == 0 )
	{
		timeoutData_m->mixDone = value;
	}

	else if ( stricmp( paramName_m, "blackOffsets" ) == 0 )
	{
		timeoutData_m->blackOffsets = value;
	}

	else if ( stricmp( paramName_m, "pass" ) == 0 )
	{
		timeoutData_m->pass = value;
	}

	else if ( stricmp( paramName_m, "analysisDone" ) == 0 )
	{
		timeoutData_m->analysisDone = value;
	}

	else if ( stricmp( paramName_m, "resultsCalcComplete" ) == 0 )
	{
...
ParseTimeouts( TimeoutData_t * timeoutData )
{
	 ...
	 */
	fp = fopen( TIMEOUTS_CONFIG_FILE_NAME, "r" );
	if ( fp == NULL )
	{
