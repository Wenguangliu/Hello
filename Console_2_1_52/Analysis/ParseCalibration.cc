...
SetParam()
{
	...
	if ( stricmp( paramName_m, "barcodeDacs" ) == 0 )
	{
	...
	else if ( stricmp( paramName_m, "cuvetteMarkDacs" ) == 0 )
	{
		s = strtok( paramValue_m, "," );
	...
	else if ( stricmp( paramName_m, "wavelengthDacTrims" ) == 0 )
	{
		s = strtok( paramValue_m, "," );
...
ParseCalibration( CalibrationData_t * calibrationData )
{
	 ...
	 */
	fp = fopen( CALIBRATION_CONFIG_FILE_NAME, "r" );
	if ( fp == NULL )
	{
...
WriteCalibration( CalibrationData_t * calibrationData )
{
	...
	// Open the file.
	fp = fopen( CALIBRATION_CONFIG_FILE_NAME, "w" );
	if ( fp == 0 )
	{
		return CFG_ERR_FILE_OPEN;
	}
