...
const char *		StateAnalysis::infoText = "";
pthread_t			StateAnalysis::timerThread = 0;
...
SetTimer( int time )
{
	...
	if ( _time >= 0 )
	{
		pthread_create( &timerThread, 0, TimerThread, &_time );
	}
}
...
TimerThread( void * arg )
{
	...
	static mqd_t	queue = mq_open( ANALYSIS_QUEUE_NAME, O_WRONLY );
...
StartAnalysis()
{
	...
	// If in Demo mode or a Rotor 100 or an uploaded rotor, trigger the run start without a barcode read.
		...
		case MSG_ENGINE_COMM_OUT_MESSAGE_RECEIVED:
			...
			switch ( engineMsg->message[0] )
			{
				...
				case 'B':
					...
					// what's specified in the order, clear uploadedOrderInProcess so we don't dequeue
					...
					if ( systemError.GetAction( errNum ) == SERR_ACTION_NONE )
					{
						...
						if ( (cfgErr == CFG_ERR_NONE) && !(isRotor1 && rotor1RunsComplete) )
						{
							...
							{
								strncpy( runInfo.rocFileName, basename( rocFileName ), sizeof( runInfo.rocFileName ) );
				...
				/*
				 * Engine Asynchronous Reset message
				 */
 ...
 * 
 * RETURNS
 *	System error number, or SERR_NONE if there is no error.
 */
...
CreateResultsDatabase()
{
	...
	// Now, open it and initialize it as an empty database.
	fdIndex = open( dbIndex, O_CREAT | O_WRONLY | O_DSYNC, 0644 );
	if ( fdIndex == -1 )
	{
	...
	write( fdIndex, &resultsIndex, sizeof( ResultsDbIndex_t ) );
	close( fdIndex );
	...
	status = truncate( dbData, RESULTS_MAX * sizeof( ResultsDbEntry_t ) );
	...
	if ( status == -1 )
	{
		...
		fdData = open( dbData, O_CREAT | O_WRONLY | O_DSYNC, 0644 );
		if ( fdData == -1 )
		{
		...
		if ( status == -1 )
		{
			LogError( "error setting results database file length" );
		}
	}
}
...
SaveResults( RotorResults_t * rotorResults, bool is655321 )
{
	...
	{
		sprintf( runInfo.patientControlId, "%s %X", strings[ RSTR_ERROR ], runInfo.errNum );
	}
...
CreateResultsRecord( ResultsDbEntry_t * resultsDbEntry, RotorResults_t * rotorResults, bool * resultSuppressed )
{
	...
	for ( i = FIRST_ANALYTE_RESULT_TYPE; i < MAX_ANALYTE_TYPES; i++ )
	{
		...
		{
			// If this is an eGFR result, do not copy if this is a piccolo or if eGFR is not enabled or this is a Control, and this is an eGFR result.
...
//				memcpy( &resultsDbEntry->rotorAnalyteResults[ i ], &rotorResults->rotorAnalyteResult[ i ], sizeof( RotorAnalyteResult_t ) );
	...
	{
		strcpy( resultsDbEntry->rotorName, strings[ RSTR_UNKNOWN ] );
	...
	if ( (factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE) && (t > 0) && (t < time( 0 )) )
	{
...
//		 resultSuppressed )
		 ...
		 (settingsData->errorPrintAuto == false) )
	{
		...
		{
			LogDebug( "  system error %h", runInfo.errNum );
		}
		...
		{
			errNum = ResultsPrint::PrintErrorReport( ndx, abbreviated );
...
SendRisFile( int rotorId )
{
	...
	strncpy( runInfo.risFileName, basename( risFileName ), sizeof( runInfo.risFileName ) );
...
CalculateAge()
{
	...
	{	
		...
		age = currentTime->tm_year + 1900 - runInfo.dateOfBirth.year;

		// If it's currently an earlier month than the birth month, or an earlier day in the same month, then decrement the age.
		if ( (currentTime->tm_mon < runInfo.dateOfBirth.month) ||
			 ((currentTime->tm_mon == runInfo.dateOfBirth.month) && (currentTime->tm_mday < runInfo.dateOfBirth.day)) )
		{
			age--;
		}
	}
...
Enqueue( UploadedOrder_t * order )
{
	...
	// Set the order ID to the current system time.
	order->orderId = time( 0 );
	
	if ( ordersList->numOrders < MAX_QUEUED_ORDERS )
	{
		// Find out where in the index list our new order should go.
...
GetDisplayList( int page, OrderDisplayList_t * list )
{
	...
	// Clear the display list array, so any we don't populate will be empty.
...
SendAstmCancelMessage( UploadedOrder_t * order, char code )
{
	...
	if ( queue == 0 )
	{
		...
		if ( queue == -1 )
		{
			LogError( "failure to open %s queue.", ASTM_QUEUE_NAME );
			queue = 0;
		}
	}
	...
	if ( queue != 0 )
	{
		...
		mq_send( queue, (const char *)&msg, sizeof( AstmMsg_t ), MQ_PRIO_MAX-1 );
	}
}
