...
static char *	GetAstmDateTime( time_t time );
			  ...
			  RawData_t *			rawData )
{
	...
	fp = fopen( radFileName, "w" );
	if ( fp == NULL )
	{
		LogError( "error opening RAD file <%s> (%s)", radFileName, strerror( errno ) );
		return "";
	}
...
static char *
GetAstmDateTime( time_t time )
{
	static char	buff[ 13 ];
	struct tm *	t = gmtime( &time );
	
	sprintf( buff, "%4.4d%2.2d%2.2d%2.2d%2.2d", t->tm_year + 1900,
												t->tm_mon + 1,
												t->tm_mday,
												t->tm_hour,
												t->tm_min );
...
{
	FILE * 			fp;
	RadDbIndex_t	radDbIndex;
	
	// Open the RAD index file.
	fp = fopen( RAD_FILE_INDEX, "r+" );
	...
	if ( fp == 0 )
	{
		...
		if ( errno == ENOENT )
		{
			fp = fopen( RAD_FILE_INDEX, "w" );
			if ( fp == 0 )
			{
				LogError( "error creating RAD db (%s)", strerror( errno ) );
				return;
			}
		...
		{
			LogError( "error opening RAD db (%s)", strerror( errno ) );
			return;
		}
	}
	...
	{
		if ( unlink( radDbIndex.fileName[ radDbIndex.lastEntry ] ) != 0 )
		{
