...
ReadArchiveIndex()
{
	...
	fp = fopen( REF_RANGE_ARCHIVE_INDEX, "rb" );
	if ( fp != 0 )
	{
...
{
	char		fileName[100];	// Long enough for the full path and file name.
	FILE *		fp;
	CfgError_t	cfgErr;
	SYSTEMTIME	localST;
	FILETIME	localFT;
	...
	localTime = (((ULONGLONG) localFT.dwHighDateTime) << 32) + localFT.dwLowDateTime;

	sprintf( fileName, "%s%I64u.xml", REF_RANGES_DIRECTORY, localTime );
	LogDebug("StateArchive::ArchiveCurrentRefRanges | current time is %I64u", localTime);
	LogDebug("StateArchive::ArchiveCurrentRefRanges | filename is %s", fileName);
	...
	if ( archiveIndex->fileNames[ archiveIndex->lastEntry ][0] != '\0' )
	{
		...
		wsprintf(wFileName, L"%S", archiveIndex->fileNames[ archiveIndex->lastEntry ]);
		//if ( unlink( archiveIndex->fileNames[ archiveIndex->lastEntry ] ) != 0 )
		if ( DeleteFile( wFileName ) == FALSE )
		{
	...
	fp = fopen( REF_RANGE_ARCHIVE_INDEX, "wb" );
	if ( fp != 0 )
	{
...
GetArchiveLabel( char * buff, char * fileName )
{
	...
	if (baseFileName != NULL) {
		baseFileName++;
		fileTime = _atoi64(baseFileName);
	}
	else
	{
		fileTime = _atoi64(fileName);
	}
	t.dwHighDateTime = (fileTime >> 32);
	t.dwLowDateTime = (fileTime);
	...
	sprintf( buff, "%d %s %d %d:%02d %s", tm.wDay,
										  WstrToCstr (strings.Month( tm.wMonth )),
										  tm.wYear,
										  hour,
										  tm.wMinute,
										  suffix );
...
Enter()
{
	...
	wcscpy( data->buttonText[0], strings[ MSTR_SEND ] );
	wcscpy( data->buttonText[1], strings[ MSTR_RETRIEVE ] );
...
Enter()
{
	...
	memset( data, 0, sizeof( GuiMiddleButtonsInfoData_t ) );
	wcscpy( data->buttonText[0], strings[ MSTR_PRINTER ] );
	wcscpy( data->buttonText[1], strings[ MSTR_INTERNAL_ARCHIVE ] );
	wcscpy( data->buttonText[2], strings[ MSTR_EXTERNAL_ARCHIVE ] );
...
Enter()
{
	...
	{
		wcscpy( data->items[i][0], CstrToWstr (archiveList.names[ i ]) );
	}	
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
ProcessMessage( int msgId, char * data )
{
	...
	if ( doUpdate )
	{
		...
		{
			wcscpy( data->items[ i ][0], CstrToWstr (archiveList.names[ topItemArchiveRetrieve + i ]) );
		}	
...
Enter()
{
	...
	wsprintf( buff, L"%s %S", strings[ ESTR_RESTORING_REF_RANGES ], selectedArchiveLabel );
...
Enter()
{
	...
	wcscpy( data->buttonText[0], strings[ MSTR_ALL ] );
	if ( factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE )
	{
		wcscpy( data->buttonText[1], strings[ SSTR_PATIENT ] );
		wcscpy( data->buttonText[2], strings[ SSTR_CONTROL ] );
	}
	else
	{
		wcscpy( data->buttonText[1], strings[ SSTR_DOG ] );
		wcscpy( data->buttonText[2], strings[ SSTR_CAT ] );
		wcscpy( data->buttonText[3], strings[ SSTR_HORSE ] );
		wcscpy( data->buttonText[4], strings[ SSTR_OTHER ] );
	}
...
Enter()
{
	...
	{
		wcscpy( data->items[0][0], strings[ MSTR_ALL ] );
		i = 1;
	}
		
	for ( ; i < 5 && i < activeRanges->len - topItemSelectReport + 1; i++ )
	{
		wcscpy( data->items[ i ][0], species.GetSampleTypeString (activeRanges->list[ topItemSelectReport + i - 1 ], &strings ) );
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
Enter()
{
	...
	if ( doPrint )
	{
        printerMsg.sampleType = selectedType;
        printerMsg.msgId     = MSG_PRINT_REFERANCE;
        errNum = SendPrinterMessage (&printerMsg);
        
	}// no ASTM and HL7 sendings
	else if (( settingsData->comm.commInterface != COMM_ASTM ) && (settingsData->comm.commInterface != COMM_HL7))
	{
