...
RotorExpired( char month, char day, char year )
{
	...
	// of years since 2000.	
	if ( currentDate.wYear - 2000 > year )
	{
		return true;
	}
	...
	if ( currentDate.wYear - 2000 < year - 3 )
	{
		return true;
	}
	...
	if ( (currentDate.wYear - 2000 == year) && (currentDate.wMonth > month) )
	{
		return true;
	}
	...
	if ( (currentDate.wYear - 2000 == year) &&
		(currentDate.wMonth == month) && 
		(currentDate.wDay > day) )
	{
		return true;
	}
...
CalcRotorAgeInDays( MfgLot_t lotInfo )
{
	...
	currentJulian = JulianDate( currentDate.wMonth, currentDate.wDay, currentYear );
...
int
ProcessRotorBarcode (char *msgPrt, unsigned short *barcodeFactor, BarcodeData_t * barcodeData)
{
	char		    barcodeDigits [NUM_BARCODE_DIGITS];
	int 	        errNum = ERR_BC_NONE;
	FILE *		    fp;
	int			    i;
    unsigned short *barcodeTransitions;
    ...
    barcodeTransitions = (unsigned short *)(msgPrt + BARCODE_PARAMETER_OFFSET);

    // get barcode read parameters
    for (i =0; i < 8; i++) {
        barcodeFactor [i] = barcodeTransitions [i]; 
        LogDebug ("0x%04x", barcodeFactor [i]);
    }

    barcodeTransitions = (unsigned short *)(msgPrt + BARCODE_DATA_OFFSET);

    fp = fopen (BC_TRANSITIONS_FILE, "wb");
	if (fp == 0) {
