...
// (also defined in AlgGlobals.h - should these be in CommonDefines.h?

#define	SYS_ILLEGAL_LOG_PARAM			0x403c		// math	error in system	calcs 
#define	SYS_CHECK_DIVIDE_BY_0			0x403f		// system check	divide by 0	error
#define	MISSING_FILTER_CURVE_FACTORS 	0x406b		
 
#define	NO_ERROR_NUMBER					0
					...
					unsigned short *		errorNumber )
{
	...
	*filterSum = 0.0;
	...
	if (dyeCenter <= 0.0)
	{
		*errorNumber = SYS_ILLEGAL_LOG_PARAM;
		return;
	}

	*dyeAbs = (float)(-log10(dyeCenter));

	// accummulate (wavelength filter cals) & (wavelength filter cals * dye cals)

	for (i = 0; i < MAX_BFD_POINTS; i++)
	{
		filter = FactoryData.wavelengthFilterCurves[filterIdx][i];

		if (filter == UNINITIALIZED_FILTER_CAL)
		{
			*errorNumber = MISSING_FILTER_CURVE_FACTORS; 
			return;
		}
	...
	if (*filterSum == 0.0)
	{
		*errorNumber = SYS_CHECK_DIVIDE_BY_0;
		return;
	}
					   ...
					   unsigned short *	errorNumber )
{
	...
	*calFactor = 1.0;     
	...
	if (*errorNumber != NO_ERROR_NUMBER)
	{
		return;
	}
...
void CalcBfds (void)
{
	...
	for (i = 0; i < MAX_BFD_CALS; i++)
	{
		...
		if ( errorNumber != NO_ERROR_NUMBER )
		{
			printf( "AmsRxComm: error %x calculating BFD[%d]\n", errorNumber, i );
			calFactor = 1.0; 
		}
