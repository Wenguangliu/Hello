...
SystemErrorNumber_t
ProcessRotorBarcode (const unsigned short * barcodeTransitions, unsigned short *barcodeParameters, BarcodeData_t * barcodeData, char *digitalFileName)
{
	...
	{
		fp = fopen( BC_TRANSITIONS_FILE, "w+b" );
		if ( fp == 0 )
		{
	...
	{
        // save parameters for ams test
        if (strcmp (digitalFileName, AMS_BARCODE_DATA_FILE) == 0) {
            for (i = 0; i < NUM_BARCODE_PARAMETERS; i++) {
                fprintf (fp, "0x%04x", barcodeParameters [i]);
            }
        }
