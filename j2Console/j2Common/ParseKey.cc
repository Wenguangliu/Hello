...
SetParam()
{
	if ( _stricmp( paramName_m, "serialNumber" ) == 0 )
	{
		strcpy( keyData_m->serialNumber, paramValue_m );
	}

	else if ( _stricmp( paramName_m, "mode" ) == 0 )
	{
		if ( _stricmp( paramValue_m, "Demo" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_DEMO;
		}
		else if ( _stricmp( paramValue_m, "Research" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_RESEARCH;
		}
		else if ( _stricmp( paramValue_m, "Manufacturing" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_MANUFACTURING;
		}
		else if ( _stricmp( paramValue_m, "Service" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_SERVICE;
		}
		else if ( _stricmp( paramValue_m, "Administrator" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_ADMINISTRATOR;
		}
		else if ( _stricmp( paramValue_m, "Lockout" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_LOCKOUT;
		}
		else if ( _stricmp( paramValue_m, "RandD" ) == 0 )
		{
			keyData_m->mode = KEY_MODE_R_AND_D;
		}
	}
	else if ( _stricmp( paramName_m, "enableFtpTelnet" ) == 0 )
	{
		if ( _stricmp( paramValue_m, "true" ) == 0 )
		{
    ...
    else if (_stricmp( paramName_m, "demoFromKey" ) == 0 )
    {
        if ( _stricmp( paramValue_m, "true" ) == 0 )
        {
    ...
    else if ( _stricmp( paramName_m, "useFastDemo" ) == 0 )
    {
        if ( _stricmp( paramValue_m, "true" ) == 0 )
        {
...
CfgError_t
ParseKey( const char * path, KeyData_t * keyData )
{
	unsigned	version;

	return ParseKey( path, keyData, &version );
}

CfgError_t
ParseKey( const char * path, KeyData_t * keyData, unsigned * version )
{
	FILE *		fp;
	int			done;
	char		buff[ BUFF_SIZE ];
	 ...
	 */
	sprintf( fileName, "%s/%s", path, KEY_CONFIG_FILE_NAME );
