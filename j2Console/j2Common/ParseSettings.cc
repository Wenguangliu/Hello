...
SetParam()
{
	...
	else if ( _stricmp( paramName_m, "commAnalyzerIp" ) == 0 )
	{
		strcpy( settingsData_m->comm.analyzerIp, paramValue_m );
	}
	
	else if ( _stricmp( paramName_m, "commGateway" ) == 0 )
	{
		strcpy( settingsData_m->comm.gateway, paramValue_m );
	}
	
	else if ( _stricmp( paramName_m, "commIpMask" ) == 0 )
	{
	...
	else if ( _stricmp( paramName_m, "commClientSocket" ) == 0 )
	{
		settingsData_m->comm.clientSocket = atoi( paramValue_m );
	}
	...
	else if ( _stricmp( paramName_m, "hostName" ) == 0 )
	{
        strcpy( settingsData_m->comm.hostName, paramValue_m );
	}
...
ParseSettings (SettingsData_t * settingsData, unsigned * version)
{
	 ...
	 */
	fp = fopen( SETTINGS_CONFIG_FILE_NAME, "rb" );
	if ( fp == 0 )
	{
...
WriteSettings( SettingsData_t * settingsData )
{
	...
	// Open the file.
	fp = fopen( SETTINGS_CONFIG_FILE_NAME, "wb" );
	if ( fp == 0 )
	{
		return CFG_ERR_FILE_OPEN;
	}
	...
	// commInterface
	sprintf( dataBuff, "%d", settingsData->comm.commInterface );
	...
	// commClientSocket
	sprintf( dataBuff, "%d", settingsData->comm.clientSocket );
	fprintf( fp, "    <commClientSocket>%s</commClientSocket>\n", dataBuff );
	crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );

	// dhcp  
	sprintf( dataBuff, "%d", settingsData->comm.dhcpEnabled );
	fprintf( fp, "    <dhcpEnabled>%s</dhcpEnabled>\n", dataBuff );
	crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );

	// hostName 
	sprintf( dataBuff, "%s", settingsData->comm.hostName );
    ...
    // fanIdleSpeed
    sprintf( dataBuff, "%d", settingsData->fanIdleSpeed );
    fprintf( fp, "    <fanIdleSpeed>%s</fanIdleSpeed>\n", dataBuff );
    crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );

    sprintf( dataBuff, "%d", settingsData->hl7ConfigIdx );
    fprintf( fp, "    <hl7ConfigIdx>%s</hl7ConfigIdx>\n", dataBuff );
    crc = Crc32Update( crc, (unsigned char *)dataBuff, strlen( dataBuff ) );
