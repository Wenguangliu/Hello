...
SetRotorCalculationRecordParam()
{
	...
	if ( _stricmp( paramName_m, "hemolyzed340InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->hemolyzed340InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "lipemic340InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->lipemic340InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "icteric340InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->icteric340InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "hemolyzed405InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->hemolyzed405InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "lipemic405InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->lipemic405InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "icteric405InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->icteric405InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "hemolyzed467InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->hemolyzed467InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "lipemic467InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->lipemic467InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "icteric467InvertedRatioMatrix" ) == 0 )
	{
		ParseFloatArray( rcr->icteric467InvertedRatioMatrix, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "hemolyzedSlope" ) == 0 )
	{
		ParseFloatArray( rcr->hemolyzedSlope, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "hemolyzedIntercept" ) == 0 )
	{
		ParseFloatArray( rcr->hemolyzedIntercept, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "lipemicSlope" ) == 0 )
	{
		ParseFloatArray( rcr->lipemicSlope, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "lipemicIntercept" ) == 0 )
	{
		ParseFloatArray( rcr->lipemicIntercept, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "ictericSlope" ) == 0 )
	{
		ParseFloatArray( rcr->ictericSlope, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "ictericIntercept" ) == 0 )
	{
		ParseFloatArray( rcr->ictericIntercept, NUM_SB_MATRIX );
	}
	else if ( _stricmp( paramName_m, "rotorBeadMissingCheckLimit" ) == 0 )
	{
		rcr->rotorBeadMissingCheckLimit = (unsigned)atoi( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorDistributionCheckLimit" ) == 0 )
	{
		rcr->rotorDistributionCheckLimit = (unsigned)atoi( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorDilutionCheckEnabled" ) == 0 )
	{
		rcr->rotorDilutionCheckEnabled = (unsigned)atoi( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorShortSampleProbThreshold" ) == 0 )
	{
		rcr->rotorShortSampleProbThreshold = (float)atof( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorLongSampleProbThreshold" ) == 0 )
	{
		rcr->rotorLongSampleProbThreshold = (float)atof( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorFlags" ) == 0 )
	{
		ParseUnsignedArray( rcr->rotorFlags, NUM_ROTOR_FLAGS_LIMITS );
	}
	else if ( _stricmp( paramName_m, "rotorFactorsLimits" ) == 0 )
	{
		ParseFloatArray( rcr->rotorFactorsLimits, NUM_ROTOR_FLAGS_LIMITS );
	}
	else if ( _stricmp( paramName_m, "cuvettePathLength" ) == 0 )
	{
		index = -1;
		if ( attributes_m[0] != 0 && _stricmp( attributes_m[0], "index" ) == 0 && attributes_m[1] != 0 )
		{
...
SetBarcodeFactorParam( AnalyteCalcRecord_t * acr )
{
	if ( barcodeFactorIndex_m < NUM_BARCODE_FACTORS )
	{
		...
		else if ( _stricmp( paramName_m, "scale" ) == 0 )
		{
			acr->barcodeFactor[ barcodeFactorIndex_m ].scale = (float)atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "offset" ) == 0 )
		{
...
SetParam()
{
	...
	else if ( _stricmp( paramName_m, "rotorNumber" ) == 0 )
	{
		rocData_m->rotorNumber = (unsigned char)atoi( paramValue_m );
	}
	else if ( _stricmp( paramName_m, "rotorFormat" ) == 0 )
	{
	...
	else if ( _stricmp( paramName_m, "rotorPrinterName" ) == 0 )
	{
...
StartTag( void *data, const char *el, const char **attr )
{
	...
	if ( inData_m )
	{
		if ( _stricmp( el, "rotorCalculationRecord" ) == 0 )
		{
			inRotorCalculationRecord_m = true;
		}
		
		else if ( _stricmp( el, "analyteCalculationRecord" ) == 0 )
		{
		...
		else if ( _stricmp( el, "barcodeFactor" ) == 0 )
		{
			if ( attr[0] != 0 && _stricmp( attr[0], "index" ) == 0 && attr[1] != 0 )
			{
...
EndTag( void *data, const char *el )
{
	...
	if ( inData_m )
	{
		...
		else if ( _stricmp( el, "rotorCalculationRecord" ) == 0 )
		{
