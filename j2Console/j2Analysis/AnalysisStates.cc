...
#define RUNNER		                L"\\Hard Disk\\Abaxis\\bin\\UpdateEngineSoftware.exe"
#define NOTIFICATION_PROCESS        L"/Hard disk/Abaxis/Util/Notification.exe"
#define SHUTDOWN_PROCESS            L"shutdown.exe"
const wchar_t  *updateEngStr        =L"Engine Code Out of Sync\nUpdating Engine Code...";
const wchar_t  *updateEngErrStr     =L"Engine Code Update Fail\n Contact Tech Support"; 

// for analysis test only
//#define  SKIP_ROTOR_PRESENT_DETECT

#ifdef SIMU_DRAWER_TEST
#define DEBUG_DRAWER_OPEN_TIME  4 
#define DEBUG_TIME_DELAY        10000
#endif

#define  NEW_ICON
#undef   CANCEL_SELECTION     // for analyzer noise debug used only

#undef   ASTM_DEBUG
#undef   DEMO_SHOW          // disable cancel and analyzer button for demo show
...
ProcessMessage( int msgId, char * data )
{
	...
	if ( msgId == MSG_ANALYSIS_TRIGGER )
	{
        ...
        //ZZZ should not set this variable to be 0
		...
		if ( elapsedTime < 2 )
		{
			Sleep( (elapsedTime > 0 ? 2 - elapsedTime : 2) *1000 );
		}
...
Enter()
{
	...
	else
	{
		...
		{
			wsprintf( topLabel, L"%s %S", strings[ TSTR_CONTROL ], runInfo.patientControlId );
		}
		else
		{
			wsprintf( topLabel, L"%s %S", strings[ TSTR_PATIENT ], runInfo.patientControlId );
		}

		SetTopLabel( topLabel );

		msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
		wcscpy( msg.data.middleMessage.message, strings[ MSTR_ANALYSIS_COMPLETE ] );
		SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
...
Enter()
{
	...
	if ( receivedRotorName || rotorNameValid )
	{
		...
		wcscpy( msg.data.middleProgBarMessage.progBarText, strings[ MSTR_ANALYZING_SAMPLE ] );
		memset (rName1, 0, sizeof (rName1));
        memset (rName2, 0, sizeof (rName2));
        wcscpy (rName1, CstrToWstr(rotorConfig->rotorScreenNameLine1 ));
        wcscpy (rName2, CstrToWstr(rotorConfig->rotorScreenNameLine2));

        wsprintf(msg.data.middleProgBarMessage.message, L" %s \n %s ",
                    rName1, rName2);
	...
	else
	{
		...
		wcscpy( msg.data.middleProgBarMessage.progBarText, strings[ MSTR_ANALYZING_SAMPLE ] );
		wcscpy( msg.data.middleProgBarMessage.message, strings[ MSTR_DETERMINING_ROTOR_TYPE ] );
...
#ifdef SIMU_DRAWER_TEST
    CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)DebugTimerThread, NULL, 0, NULL);
#endif
...
void *
StateAnalysisInProcess::
DebugTimerThread (void *arg)
{
    const int       msg = MSG_GUI_OUT_BOTTOM_BUTTON;
    ...
    // wait for 5 seconds to cancel analyzing
    Sleep(DEBUG_TIME_DELAY);
...
ProcessMessage( int msgId, char * data )
{
	...
	switch ( msgId )
	{
		case MSG_ENGINE_COMM_OUT_MESSAGE_RECEIVED:
			...
			{
#ifndef   CANCEL_SELECTION
				retVal = STATE_OPEN_DRAWER_WITH_ROTOR;
#else
				retVal = STATE_CANCELLED_SELECTION;
#endif
			}
			else
			{
...
// create for noise test purpose
// when analysis is cancelled, user coudl be  eigher 
// select open drawer or run again without load rotor
void
StateCancelledSelection::
Enter()
{
	GuiReceiveMsg_t msg;
    GuiMiddleButtonsData_t  *data = &msg.data.middleButtons;

	SetTopLabel(L"Cancelled Selection");
    memset (data, 0, sizeof (GuiMiddleButtonsData_t));
    wcscpy (data->buttonText[0], L"Open Drawer");
    wcscpy (data->buttonText[1], L"Run  Again");
...
ProcessMessage( int msgId, char * data )
{
	int 	retVal = STATE_CANCELLED_SELECTION;
    int     idata = *(int *)data;
	...
	switch (msgId)
	{
		case MSG_GUI_OUT_BUTTON:
             switch (idata) {
                ...
                case 1: // re-run rotor 
                        StartAnalysis();
                        if ( rotor1RunsComplete && isRotor1 )
                        {
                            retVal = STATE_ANALYSIS_COMPLETE;
                        }
		...
		case MSG_ENGINE_COMM_OUT_MESSAGE_RECEIVED:
			...
			if ( engineCommMsg->message[0] == 'D' )
			{
				...
				else if ( status & 0x10 )	// Drawer closed
				{
					...
					{
						// clear out Rotor 1/100 flags.
						isRotor1 = false;
						isRotor100 = false;

                        // Clear the runInfo for Rotor 100, but not for Rotor 1
                        memset( &runInfoRotor100, 0, sizeof( RunInfo_t ) );
                        // detect database check file
                        hSearch = FindFirstFile (TEXT (DATABASE_CHECK_FILE_NAME),  &fileData);
                        // no database check file, allow to check database file size
                        if (hSearch == INVALID_HANDLE_VALUE) {
                            if (GetDataBaseFileSize (factoryData, false, systemData->maxDataBaseSize) == 1) {
                                analysisWarningQueue.Enqueue (SERR_DATABASE_FILE_TOO_LARGE);
...
Enter()
{
    ...
    msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
    wcscpy( msg.data.middleMessage.message, strings[ MSTR_CANCEL_ORDER ] );
    SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
...
Enter()
{
    ...
    UploadedOrder_t *   orderInfo = orderQueue.GetEntryInfo( uploadedOrderSelected );
     ...
     // ZZZ
    if (orderInfo == NULL) {
        LogDebug ("could not be able to find out order ID  %lu", uploadedOrderSelected);
    }
    ...
    // prevent crash
    if (orderInfo != NULL) {
        wcscpy( msg.data.middleLoadOrder.orderLabel, CstrToWstr (orderInfo->orderLabel) );
	    memset (rName1, 0, sizeof (rName1));
        memset (rName2, 0, sizeof (rName2));
        wcscpy (rName1, CstrToWstr( orderInfo->rotorNameLine1 ));
        wcscpy (rName2, CstrToWstr( orderInfo->rotorNameLine2));
	    wsprintf( msg.data.middleLoadOrder.rotorName, L" %s \n %s ",
                    rName1, rName2);
    ...
    wcscpy( msg.data.middleLoadOrder.buttonText, strings[ MSTR_CANCEL_ORDER ] );
...
Enter()
{
	...
	wcscpy( msg.data.middleEnterDate.textLeft, L"0" );
	wcscpy( msg.data.middleEnterDate.textMiddle, L"0" );
	wcscpy( msg.data.middleEnterDate.textRight, strings[ MSTR_YRS ] );
...
ProcessMessage( int msgId, char * data )
{
	...
	else if ( updateTens )
	{
		...
		wsprintf( msg.data.middleEnterDate.textLeft, L"%d", ageValue / 10 );
		SendGuiMessage( &msg, sizeof( GuiMiddleEnterDateData_t ) );
	}
	...
	else if ( updateOnes )
	{
		...
		wsprintf( msg.data.middleEnterDate.textMiddle, L"%d", ageValue % 10 );
		SendGuiMessage( &msg, sizeof( GuiMiddleEnterDateData_t ) );
	}
	...
	else if ( updateUnits )
	{
		...
		wcscpy( msg.data.middleEnterDate.textRight, strings[ sid ] );
...
ProcessMessage( int msgId, char * data )
{
	...
	else if ( updateYear )
	{
		...
		wsprintf( updateMsg.data.middleEnterDate.textRight, L"%04d", year );
		SendGuiMessage( &updateMsg, sizeof( GuiMiddleEnterDateData_t ) );
	}
		...
		case MSG_GUI_OUT_ID_TEXT:
			...
			if ( (factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE) && (expirys.NumOps() > 0) )
			{
				...
				if ( expiry != 0 )
				{
					...
					if ( (expiry->expiration.dwLowDateTime != 0 || expiry->expiration.dwHighDateTime != 0) && (CompareFileTime(&(expiry->expiration), &currentFT) == -1))
					{
	...
	switch ( msgId )
	{
		...
		case MSG_GUI_OUT_ID_TEXT:
...

			{
				if ( sdata[0] == '\0' )
				{
					strcpy( runInfo.patientControlId, WstrToCstr (strings[ MSTR_NONE ]) );
				}
				else
				{
...
Enter()
{
    ...
    else if ( systemData->inRandDMode) 
    {
            msg.msgId =  MSG_GUI_MIDDLE_HOME_READY_RD;        
    }
	else
	{
    ...
    wcscpy( msg.data.middleHomeReady.buttonText[0], strings[ MSTR_ANALYZE ] );
    ...
    if ( ((settingsData->comm.commInterface != COMM_ASTM) && (orderQueue.NumOrders() > 0)) ||
    ...
    {
        wcscpy( msg.data.middleHomeReady.buttonText[1], strings[ MSTR_PENDING_ORDER ] );
    ...
    if ( systemData->inResearchMode == true )
    {
        ...
        {
            wcscpy( msg.data.middleHomeReady.buttonText[2], CstrToWstr (uploadedRotorName));
        }
    ...
    // Clear the uploaded order ID flag, which is set when an uploaded order is selected.
    ...
    // ZZZ  tell start up process, home is ready
    //SendStartupMessage (MSG_STARTUP_HOME_READY);
}
...
ProcessMessage( int msgId, char * data )
{
	...
	switch ( msgId )
	{
		case MSG_GUI_OUT_BUTTON:
...
#else
                    continueState = STATE_HOME_READY;
#endif
                break;
            ...
            GetLocalTime(&time);
            SystemTimeToFileTime(&time, &currentFT);
            controlExpired = ((factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE) && (t.dwLowDateTime > 0 || t.dwHighDateTime > 0) && (CompareFileTime(&t, &currentFT) == -1) );
		...
		case MSG_GUI_OUT_MANUFACTURING:
            ...
            } else if (systemData->inRandDMode == true) {
                SendManufactureMessage (MSG_MANUFACTURE_GOTO_FOREGROUND);
                retVal = STATE_HOME_READY_BACKGROUND;
            }
			break;

        case MSG_ANALYSIS_KEY_CHANGED:
            /* in the case AMS changed, add or delelet AMS icon */
            Enter ();
        break; 
...
ProcessMessage (int msgId, char * data)
{
	...
	switch ( msgId )
	{
		case MSG_ANALYSIS_GOTO_FOREGROUND:
            ...
            InitGuiComm (ANALYSIS_QUEUE_NAME);
            if (systemData->inRandDMode == true) {
                // Register to Engine Comm
                InitEngineComm (ANALYSIS_QUEUE_NAME);
            }
...
Enter()
{
    ...
    /* 
     * if engine software verision is out of date, update engine software 
     * and reboot system  ...
     */
	if (systemData->engSoftwareUpdate) {
        /* 
         *  needs to send Gui message for display the update engine code 
         */
        LogInfo ("Start update Engine software code ..."); 

        SetTopLabel( 0 );
        msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
        wcscpy( msg.data.middleMessage.message, updateEngStr);
        SendGuiMessage( &msg, sizeof  (GuiMiddleMessageData_t));
        msg.msgId = MSG_GUI_BOTTOM_NONE;
        SendGuiMessage(&msg, 0);
        //retVal = spawnlp  (P_WAIT, RUNNER, RUNNER,  0);
		retVal = CreateProcess(RUNNER, L"", NULL, NULL, FALSE, INHERIT_CALLER_PRIORITY, NULL, NULL, NULL, &proc_info);
		WaitForSingleObject(proc_info.hProcess, INFINITE);
		
		//if ((retVal == -1) ||  (WEXITSTATUS(retVal) == 98)) { 
		if (!retVal) {
            msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
            wcscpy( msg.data.middleMessage.message, updateEngErrStr);
            SendGuiMessage( &msg, sizeof  (GuiMiddleMessageData_t));
            /* update engine fail */
            LogError ("Software update fail");
        } else {
            LogInfo ("Start shutdown system");
            /* shutdown system */
            //retVal = spawnlp (P_WAIT, "shutdown", "shutdown", 0); 
			SetSystemPowerState(NULL, POWER_STATE_OFF, POWER_FORCE);
		}
	} else  if ( systemData->rotorPresent ) {
        // If there's a rotor present at start-up, we need to move to the
	    // RotorPresentWarning state.
		SendAnalysisMessage( MSG_ANALYSIS_TRIGGER );
	} else {
        // GLD
        // We need to be able to skip this state if we had a fatal error while we were
        // off in Settings or Recall.
		SetTopLabel( strings[ TSTR_HOME ] );
        ...
        {
             msg.msgId = MSG_GUI_MIDDLE_HOME_WARMING_RD;
        }
		else
		{
...
#else
		if ( systemData->inManufacturingMode || systemData->inServiceMode ||  systemData->inRandDMode)
		{
			msg.msgId = MSG_GUI_MIDDLE_HOME_WARMING_MFG;
		}
		else
		...
		wcscpy( msg.data.middleHomeWarming.progBarText, strings[ MSTR_WARMING ] );
		if ( firstTime )
		{
			progBarValue = 0;
...
ProcessMessage( int msgId, char * data )
{
	...
	{
		case MSG_ANALYSIS_TRIGGER:
#ifdef SKIP_ROTOR_PRESENT_DETECT
			retVal = STATE_HOME_READY;
#else
		...
		case MSG_ANALYSIS_TEMPERATURE:
            ...
            {
                 msg.msgId = MSG_GUI_UPDATE_HOME_WARMING_RD_PROGRESS;
            }
			else
			{
...
#endif
			currentTemp = (int)( temperatureData->temperature + 0.5 );
...
Enter()
{
    ...
    if ( uploadedOrderSelected != 0 )
    {
        ...
        msg.msgId = MSG_GUI_MIDDLE_LOAD_ORDER;
        wcscpy( msg.data.middleLoadOrder.orderLabel, CstrToWstr(orderInfo->orderLabel));
		// convert char string to wchar_t string here
		// Note: the CstrTWstr could not be called at wsprintf two times. 
        memset (rName1, 0, sizeof (rName1));
        memset (rName2, 0, sizeof (rName2));
        wcscpy (rName1, CstrToWstr(orderInfo->rotorNameLine1));
        wcscpy (rName2, CstrToWstr(orderInfo->rotorNameLine2));
        ...
        msg.data.middleLoadOrder.fontColor = orderInfo->rotorNameFontColor;
        msg.data.middleLoadOrder.backgroundColor = orderInfo->rotorNameBackgroundColor;
        msg.data.middleLoadOrder.buttonText[0] = L'\0';
        SendGuiMessage( &msg, sizeof( GuiMiddleLoadOrderData_t ) );
    }
    else
    {
        ...
        wcscpy( msg.data.middleMessage.message, strings[ MSTR_CLOSE_DRAWER_TO_ANALYZE ] );
        SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
    }
	SetBottomButtons( strings[ BSTR_CLOSE ], true );
	LogDebug ("research mode =%d", systemData->inResearchMode);
...
Enter()
{
	...
	msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
	wcscpy( msg.data.middleMessage.message, strings[ MSTR_LOAD_ROTOR_TO_ANALYZE ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
	...
	{
#ifdef SIMU_DRAWER_TEST
		SetTimer(DEBUG_DRAWER_OPEN_TIME);
#else
...
Enter()
{
    ...
    /* Cam up during warning up, send comamnd to Engine to 
     * reinitialize motor driver to work around hardware
     * problem
     */
	SendDrawerMessage( 'R' );
}
...
Enter()
{
	...
	msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
	wcscpy( msg.data.middleMessage.message, strings[ MSTR_PLEASE_REMOVE_ROTOR ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
...
Enter()
{
	...
	wcscpy( guiMsg.data.winError.topLabel, strings[ TSTR_WARNING ] );
	wcscpy( guiMsg.data.winError.message, strings[ ESTR_PLEASE_REMOVE_ROTOR ] );
...
Enter()
{
	...
	msg.msgId = MSG_GUI_MIDDLE_MESSAGE;
	wcscpy( msg.data.middleMessage.message, strings[ MSTR_SAVING_RESULTS ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleMessageData_t ) );
...
Enter()
{
	...
	{
		wcscpy( guiMsg.data.middleButtons.buttonText[3], strings[ MSTR_NEUTERED ] );
		wcscpy( guiMsg.data.middleButtons.buttonText[4], strings[ MSTR_SPAYED ] );
	}
...
Enter()
{
    ...
    {
        wcscpy( guiMsg.data.middleListColored.items[i], CstrToWstr (orderDisplayList[ i ].orderLabel) );
    ...
    switch ( msgId )
    {
        case MSG_GUI_OUT_BUTTON:
            if ( idata >= 0 && idata <= 4 )
            {
                ...
                // ZZZ take out backup order for new fix code
                //uploadedOrderIdBak = orderListBak [idata].orderId;
...
Enter()
{
	...
	memset( &guiMsg.data.middleButtons, 0, sizeof( GuiMiddleButtonsData_t ) );
	wcscpy( guiMsg.data.middleButtons.buttonText[0], strings[ MSTR_UNKNOWN ] );
	wcscpy( guiMsg.data.middleButtons.buttonText[1], strings[ MSTR_WHITE ] );
	wcscpy( guiMsg.data.middleButtons.buttonText[2], strings[ MSTR_BLACK ] );
	wcscpy( guiMsg.data.middleButtons.buttonText[3], strings[ MSTR_ASIAN_PACIFIC_ISLANDER ] );
	wcscpy( guiMsg.data.middleButtons.buttonText[4], strings[ MSTR_NATIVE_AMERICAN ] );
	wcscpy( guiMsg.data.middleButtons.buttonText[5], strings[ MSTR_HISPANIC ] );
...
Enter()
{
	...
	{
        wcscpy( data->items[ i ][0], species.GetSampleTypeString (activeRanges->list[ i ], &strings));
	}
	data->isSelectable = 1;
	data->hasUpDown = 1;
...
ProcessMessage( int msgId, char * data )
{
	...
	else if ( doUpdate )
	{
		...
		{
            wcscpy( data->items[ i ][0], species.GetSampleTypeString (activeRanges->list[ topItem + i ], &strings));
		}
		data->isSelectable = 1;
		data->hasUpDown = 1;
...
Enter()
{
	...
	if ( factoryData->abaxisProductCode == PICCOLO_PRODUCT_CODE )
	{
		wcscpy( guiMsg.data.middleButtons.buttonText[0], strings[ SSTR_PATIENT ] );
		wcscpy( guiMsg.data.middleButtons.buttonText[1], strings[ SSTR_CONTROL ] );
	}
	else
	{
		wcscpy( guiMsg.data.middleButtons.buttonText[0], strings[ SSTR_DOG ] );
		wcscpy( guiMsg.data.middleButtons.buttonText[1], strings[ SSTR_CAT ] );
		wcscpy( guiMsg.data.middleButtons.buttonText[2], strings[ SSTR_HORSE ] );
		wcscpy( guiMsg.data.middleButtons.buttonText[3], strings[ SSTR_OTHER ] );
	}
	guiMsg.data.middleButtons.buttonHighlighted[0] = 1;
	SendGuiMessage( &guiMsg, sizeof( GuiMiddleButtonsData_t ) );
	...
	switch ( action )
	{
		...
		case SERR_ACTION_SHUTDOWN:
         ...
         case SERR_ACTION_RESTART:
            ...
            topText = strings[ TSTR_CRITICAL_STOP ];
            systemData->systemStatus = SYSTEM_STATUS_NOT_READY;
            wcscpy( guiMsg.data.winError.buttonText, strings[ BSTR_RESTART ] );
            break;
...
ProcessMessage( int msgId, char * data )
{
	...
	switch ( msgId )
	{
		...
		default:
			...
			if (( action != SERR_ACTION_SHUTDOWN ) && (action != SERR_ACTION_RESTART))
			{
...
Enter()
{
	...
	wcscpy( msg.data.middleViewResults.text, strings[ ISTR_LOAD_ROTOR_1 ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleViewResultsData_t ) );
...
Enter()
{
	...
	wcscpy( msg.data.middleViewResults.text, strings[ ISTR_LOAD_ROTOR_2 ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleViewResultsData_t ) );
...
Enter()
{
	...
	wcscpy( msg.data.middleViewResults.text, strings[ ISTR_LOAD_ROTOR_3 ] );
	SendGuiMessage( &msg, sizeof( GuiMiddleViewResultsData_t ) );
...
Enter()
{
	...
	wcscpy( guiMsg.data.winWarning.message, strings[ systemError.GetScreenText( analysisWarning ) ] );
...
ProcessMessage( int msgId, char * data )
{
	...
	{
		case MSG_GUI_OUT_BOTTOM_BUTTON:		// Continue
            //ZZZ need to optimized memory and reboot system
            if (analysisWarning == SERR_DATABASE_FILE_TOO_LARGE) {
                // The Notification.exe could not be able to shutdown analzyer because the notification is disable and 
                // the clean dababase are always 0
                CreateProcess (NOTIFICATION_PROCESS, L"", NULL, NULL,FALSE, INHERIT_CALLER_PRIORITY, NULL, NULL, NULL, &proce_info);
                // wait here until notification.exe finished function 
