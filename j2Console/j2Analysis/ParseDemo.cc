...
GetAnalyteIndex( const char * str )
{
	...
	{
		if ( _stricmp( str, ANALYTE_LIST[ i ].name ) == 0 )
		{
...
SetAnalyteParam( DemoResult_t *	dr)
{
	if ( analyteNdx_m < DEMO_MAX_ANALYTES )
	{
		...
		if ( _stricmp( paramName_m, "name" ) == 0 )
		{
		...
		else if ( _stricmp( paramName_m, "value" ) == 0 )
		{
			ai->result = (float)atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "resultPrintFlags" ) == 0 )
		{
			ai->resultPrintFlags = strtol( paramValue_m, 0, 0 );
		}
		else if ( _stricmp( paramName_m, "posNegValue" ) == 0 )
		{
			ai->posNegValue = (atoi( paramValue_m ) != 0) ? true : false;
		}
        else if ( _stricmp( paramName_m, "lowDynamicRangeLimit" ) == 0 )
        {
            ai->lowDynamicRangeLimit = strtod( paramValue_m, 0 );
        }
        else if ( _stricmp( paramName_m, "highDynamicRangeLimit" ) == 0 )
        {
            ai->highDynamicRangeLimit = strtod( paramValue_m, 0 );
        }
        else if ( _stricmp( paramName_m, "lowSystemRangeLimit" ) == 0 )
        {
            ai->lowSystemRangeLimit = strtod( paramValue_m, 0 );
        }
        else if ( _stricmp( paramName_m, "highSystemRangeLimit" ) == 0 )
        {
...
SetResultParam()
{
	...
	if ( resultNdx_m < DEMO_MAX_RESULTS )
	{
		...
		else if ( _stricmp( paramName_m, "rotorName" ) == 0 )
		{
			strncpy( dr->rotorName, paramValue_m, sizeof( dr->rotorName) );
		}
		else if ( _stricmp( paramName_m, "rotorId" ) == 0 )
		{
		...
		else if ( _stricmp( paramName_m, "lotNumber" ) == 0 )
		{
			ParseLotNumber( paramValue_m, &dr->lotNum );
		}
		else if ( _stricmp( paramName_m, "hemValue" ) == 0 )
		{
			dr->hemValue = atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "lipValue" ) == 0 )
		{
			dr->lipValue = atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "ictValue" ) == 0 )
		{
			dr->ictValue = atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "RQC" ) == 0 )
		{
			dr->rqcValue = atof( paramValue_m );
		}
		else if ( _stricmp( paramName_m, "acceptableMinimum" ) == 0 )
		{
...
StartTag( void *data, const char *el, const char **attr )
{
	...
	if ( inData_m )
	{
		if ( _stricmp( el, "result" ) == 0 )
		{
			inResult_m = true;
			analyteNdx_m = 0;
		}
		
		else if ( inResult_m && (_stricmp( el, "analyte" ) == 0) )
		{
			inAnalyte_m = true;
		}
